...
### 5. 📱 Responsive Multi-Device Auto-Sizing
- **On Mobile Phones (Screens < 768px)**:
  - Automatically loads in **Mobile AR App** mode.
  - Adapts to 100% full-width, edge-to-edge native screen view without simulated notches or thick borders.
  - Docks the bottom navigation bar (`AR Camera`, `Tourist Safety`, `Scanned Vault`) cleanly with mobile safe-area insets.
- **On Laptops / Desktops (Screens ≥ 768px)**:
  - Automatically loads in **Desktop Command Center** mode.
  - Spacious multi-column layout featuring Google Maps cartography, the Scanned Vault panel, and AI Relatable Monuments recommendations.

### 6. 🎨 Traditional Heritage Light Theme
- Custom designed color palette honoring ancient Indian temple aesthetics:
  - **Warm Parchment & Sandstone**: `#FAF7F2` and `#F3ECE1`
  - **Royal Temple Terracotta**: `#9C3A16`
  - **Antique Brass & Gold**: `#B8860B`
  - **Deep Charcoal Typography**: `#241C15` for maximum readability and contrast.
  - **Serif Classical Fonts**: Google Fonts `Cinzel` and `Cormorant Garamond`.
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/<your-username>/heritage-xr.git
   cd heritage-xr
   ```
...
This repository is pre-configured with [vercel.json](file:///c:/Users/bhava/OneDrive/Desktop/Heritage/vercel.json) for instantaneous deployment:

### Option 1: Via GitHub & Vercel Dashboard (Recommended)
1. Push your code to GitHub:
   ```bash
   git add .
   git commit -m "Deploy Heritage XR to Vercel"
   git branch -M main
...
- [ ] **LiDAR Spatial Anchors**: Integrate WebXR Device API hit-testing for real-world surface anchoring.
- [ ] **Crowdsourced Inscription Translation**: Community AI-assisted translation for deciphering unknown rock carvings.
- [ ] **Offline PWA Support**: ServiceWorker caching for zero-internet functionality at remote archaeological sites.
- [ ] **IoT Beacon Checkpoints**: Bluetooth Low Energy (BLE) proximity detection along heritage safe corridors.
