// Heritage Data Store - Monuments, Scans, Stories & Safety Routes

export const HERITAGE_DATA = {
  // Preloaded Scanned Artifacts for WebXR Camera Demo
  scannedArtifacts: [
    {
      id: 'hampi-pillar',
      name: 'Eroded Sun Temple Pillar',
      location: 'Hampi, Karnataka, India',
      dynasty: 'Vijayanagara Empire (14th Century)',
      material: 'Weathered Granite',
      condition: '72% Surface Erosion',
      coordinates: [15.3350, 76.4600],
      thumbnail: '/images/hampi-pillar.jpg',
      description: 'A grand carved pillar from the Vitthala Temple complex depicting celestial musicians and mythical Yali creatures. Severely eroded by centuries of weathering.',
      inscriptions: 'Translated: "May the prosperity of King Devaraya II illuminate the carved halls of the sun god."',
      storyText: {
        en: "Step back 600 years to the bustling capital of Vijayanagara. As dusk settles over the Tungabhadra River, master sculptors chisel this sacred granite pillar. Each carved line echoed the chants of temple rituals. Though centuries of rain and wind have softened its edges, our 3D wireframe reveals the intricate crest of the royal emblem and celestial dancers hidden within the stone.",
        hi: "६०० वर्ष पीछे विजयनगर के वैभवशाली युग में चलें। तुंगभद्रा नदी के तट पर, महान मूर्तिकारों ने इस पवित्र ग्रेनाइट स्तंभ को तराशा। समय की धुंध ने भले ही इसके नक्श धुंधले कर दिए हों, परंतु यह ३डी डिजिटल पुनर्निर्माण इसके मूल स्वरूप और शाही मुहर को आपके समक्ष पुनर्जीवित करता है।",
        ta: "600 ஆண்டுகளுக்கு முந்தைய விஜயநகரப் பேரரசின் பொற்காலத்திற்குப் பயணிப்போம். துங்கபத்திரை ஆற்றங்கரையில் உருவான இப்புனிதமான கல் தூண், காலத்தின் மாற்றத்தால் தேய்மானம் அடைந்திருந்தாலும், நமது முப்பரிமாண தொழில்நுட்பம் இதன் பழங்கால அழகையும் சிற்ப வேலைப்பாடுகளையும் மீட்டுத் தருகிறது.",
        te: "600 ఏళ్ల నాటి విజయనగర సామ్రాజ్య వైభవంలోకి అడుగుపెట్టండి. తుంగభద్ర నదీ తీరాన శిల్పులు మలిచిన ఈ పవిత్ర స్తంభం, శతాబ్దాల వాతావరణ ప్రభావంతో అరిగిపోయినా, మన 3D వైర్‌ఫ్రేమ్ టెక్నాలజీ దీని అసలు రూప రూపాంతరాన్ని కళ్లముందు ఉంచుతుంది.",
        kn: "೬೦೦ ವರ್ಷಗಳ ಹಿಂದಿನ ವಿಜಯನಗರ ಸಾಮ್ರಾಜ್ಯದ ವೈಭವಕ್ಕೆ ಪಯಣಿಸಿ. ತುಂಗಭದ್ರಾ ತೀರದಲ್ಲಿ ಕಲ್ಲಿನಲ್ಲಿ ಕೆತ್ತಲಾದ ಈ ಪವಿತ್ರ ಕಂಬವು ಕಾಲಾನಂತರದಲ್ಲಿ ಸವೆದುಹೋಗಿದ್ದರೂ, ನಮ್ಮ ೩ಡಿ ತಂತ್ರಜ್ಞಾನವು ಇದರ ನೈಜ ಸೌಂದರ್ಯವನ್ನು ಮರುಸೃಷ್ಟಿಸುತ್ತದೆ.",
        bn: "৬০০ বছর পেছনে বিজয়নগর সাম্রাজ্যের সমৃদ্ধ যুগে ফিরে যান। সময় ও আবহাওয়ার ধাক্কায় এই গ্রানাইট স্তম্ভটি ক্ষয়প্রাপ্ত হলেও, আমাদের ৩ডি প্রযুক্তির মাধ্যমে এর প্রাচীন জাঁকজমক পুনরায় দৃশ্যমান হয়েছে।",
        es: "Retroceda 600 años hasta el floreciente Imperio Vijayanagara. Este pilar de granito sagrado, desgastado por siglos de viento y lluvia, cobra vida nuevamente gracias a nuestra reconstrucción holográfica en 3D.",
        fr: "Remontez 600 ans en arrière au cœur du puissant Empire Vijayanagara. Ce pilier en granite sacré, érodé par les siècles, retrouve sa splendeur d'origine grâce à notre reconstitution 3D interactive."
      },
      meshType: 'pillar',
      relatableIds: ['brihadeeswarar', 'konark-sun', 'ajanta-caves']
    },
    {
      id: 'ashoka-edict',
      name: 'Faded Ashokan Rock Edict',
      location: 'Junagadh, Gujarat, India',
      dynasty: 'Maurya Empire (250 BCE)',
      material: 'Granite Rock Inscription',
      condition: '84% Inscription Decay',
      coordinates: [21.5222, 70.4579],
      thumbnail: '/images/ashoka-edict.jpg',
      description: 'Major Rock Edict of Emperor Ashoka inscribed in Brahmi script, decreeing non-violence, medical care for humans and animals, and moral governance.',
      inscriptions: 'Brahmi Script deciphered: "Beloved-of-the-Gods, King Piyadasi, desires that all religions reside everywhere in peace."',
      storyText: {
        en: "Imagine standing before Emperor Ashoka's royal scribes 2,200 years ago. Armed with iron chisels, they carved messages of peace and dhamma directly into raw mountain rock. Over millennia, moss and weather faded the ancient Brahmi script. Through our WebXR spectral scanner, the hidden incisions glow bright blue, unearthing Ashoka's timeless plea for harmony.",
        hi: "२२-सौ वर्ष पूर्व सम्राट अशोक के शाही शिलालेख के समक्ष खड़े हों। ब्राह्मी लिपि में उकेरे गए शांति और धर्म के संदेशों को समय की धूल ने छिपा दिया था। हमारे वेब-एक्स-आर स्कैनर से ये गुप्त रेखाएं पुनः जीवंत हो उठती हैं।",
        ta: "2200 ஆண்டுகளுக்கு முன்பு பேரரசர் அசோகரின் பிராமி எழுத்துச் சாசனங்கள் பொறிக்கப்பட்ட இப்பாறையை நமது WebXR தொழில்நுட்பம் மீண்டும் துல்லியமாக வாசித்து அதன் அமைதிச் செய்தியை வெளிப்படுத்துகிறது.",
        te: "2200 సంవత్సరాల క్రితం అశోక చక్రవర్తి బ్రాహ్మీ లిపిలో చెక్కించిన ఈ శాసనం ధర్మాన్ని, శాంతిని చాటిచెబుతుంది. మన కెమెరా స్కానర్ ఈ నిగూఢ అక్షరాలను స్పష్టంగా వెలుగులోకి తెస్తుంది.",
        kn: "೨೨೦೦ ವರ್ಷಗಳ ಹಿಂದೆ ಸಾಮ್ರಾಟ್ ಅಶೋಕರ ಬ್ರಾಹ್ಮಿ ಲಿಪಿಯ ಶಾಸನಗಳು ಕಲ್ಲಿನಲ್ಲಿ ಮೂಡಿದವು. ನಮ್ಮ ಆರ್‌ಎಕ್ಸ್ ಕ್ಯಾಮೆರಾ ತಂತ್ರಜ್ಞಾನವು ಈ ಅಳಿಸಿಹೋದ ಅಕ್ಷರಗಳನ್ನು ಮರು ಪ್ರಕಾಶಗೊಳಿಸುತ್ತದೆ.",
        bn: "২২০০ বছর পূর্বে সম্রাট অশোকের ব্রাহ্মী লিপিতে লিখিত শান্তির বাণী পুনরায় উন্মোচিত হচ্ছে আমাদের উন্নত প্রযুক্তিগত স্ক্যানারের সাহায্যে।",
        es: "Sienta la presencia del emperador Ashoka hace más de 2200 años. Las escrituras Brahmi grabadas en la roca cobran vida revelando edictos de paz y compasión.",
        fr: "Découvrez l'édit de l'empereur Ashoka vieux de 2 200 ans. L'écriture Brahmi effacée s'illumine pour révéler ses célèbres décrets de paix et d'humanisme."
      },
      meshType: 'tablet',
      relatableIds: ['sanchi-stupa', 'mahabalipuram', 'khajuraho']
    },
    {
      id: 'chola-inscription',
      name: 'Weathered Chola Inscription Slab',
      location: 'Thanjavur, Tamil Nadu, India',
      dynasty: 'Chola Dynasty (11th Century)',
      material: 'Black Basalt Relief',
      condition: '65% Text Faded',
      coordinates: [10.7828, 79.1318],
      thumbnail: '/images/chola-inscription.jpg',
      description: 'Detailing land grants, naval victories of Rajendra Chola I, and endowments to temple dancers and grain merchants in ancient Vatteluttu script.',
      inscriptions: 'Vatteluttu translated: "To the eternal glory of Lord Shiva and the brave naval commanders who crossed the eastern seas."',
      storyText: {
        en: "Feel the ocean breeze of the 11th century as King Rajendra Chola's naval fleets returned triumphant from Maritime Southeast Asia. Royal scribes hammered this basalt slab to record maritime trade routes and naval heroics. Our wireframe mesh recovers lost depth contours, bringing ancient Tamil Vatteluttu scripts into vibrant clarity.",
        hi: "११वीं शताब्दी के चोल राजवंश की समुद्री सफलताओं की कहानी। राजा राजेंद्र चोल के काल की यह शिला लिपि प्राचीन व्यापारिक मार्गों का प्रामाणिक विवरण देती है।",
        ta: "11-ஆம் நூற்றாண்டில் ராஜேந்திர சோழனின் கடற்படை வெற்றிகளையும், தஞ்சைப் பெருவுடையார் கோயிலுக்கு வழங்கப்பட்ட நிவந்தங்களையும் விவரிக்கும் வட்டெழுத்துச் சாசனம் இது. நமது 3D தொழில்நுட்பம் இச்சாசனத்தை முழுமையாக மீட்டெடுக்கிறது.",
        te: "11వ శతాబ్దపు చోళ నావికా దళాల విజయగాథను తెలిపే శాసనం. నౌకా వాణిజ్యం మరియు తమిళ వట్టెళుత్తు లిపిని డిజిటల్ విధానంలో స్పష్టంగా చూడవచ్చు.",
        kn: "೧೧ನೇ ಶತಮಾನದ ಚೋಳ ರಾಜವಂಶದ ನೌಕಾಪಡೆಯ ಸಾಹಸಗಾಥೆಯನ್ನು ವಿವರಿಸುವ ಈ ಶಾಸನವು ನಮ್ಮ ವೈರ್‌ಫ್ರೇಮ್ ಮಾದರಿಯಿಂದ ಮರುಸೃಷ್ಟಿಯಾಗಿದೆ.",
        bn: "একাদশ শতাব্দীর চোল নৌবাহিনীর গৌরবময় বিজয়ের ইতিহাস এই তামিল খোদাইচিত্রের মাধ্যমে উন্মোচিত হয়েছে।",
        es: "Viaje a las victoriosas campañas navales del Imperio Chola en el siglo XI. Las inscripciones grabadas en basalto revelan antiguas rutas comerciales.",
        fr: "Explorez les exploits maritimes de la dynastie Chola au XIe siècle. Notre scan 3D révèle les anciennes écritures gravées dans le basalte."
      },
      meshType: 'wall',
      relatableIds: ['brihadeeswarar', 'mahabalipuram', 'hampi-pillar']
    }
  ],

  // Interactive Desktop Map Monument Locations
  mapMonuments: [
    {
      id: 'brihadeeswarar',
      name: 'Brihadeeswarar Temple',
      location: 'Thanjavur, Tamil Nadu',
      dynasty: 'Chola Dynasty (1010 CE)',
      coordinates: [10.7828, 79.1318],
      category: 'Temple Architecture',
      rating: 4.9,
      image: '/images/brihadeeswarar.jpg',
      summary: 'UNESCO World Heritage Site built by Raja Raja Chola I. Features a 216-foot vimana tower constructed entirely of granite without mortar.',
      highlights: ['Single stone 80-ton Kumbam capstone', 'Ancient shadow-less architectural alignment', 'Vatteluttu mural inscriptions'],
      wireframeAvailable: true,
      audioStoryAvailable: true
    },
    {
      id: 'hampi-ruins',
      name: 'Hampi Sacred Ruins',
      location: 'Vijayanagara, Karnataka',
      dynasty: 'Vijayanagara Empire (1336 CE)',
      coordinates: [15.3350, 76.4600],
      category: 'Ancient City Citadel',
      rating: 4.9,
      image: '/images/hampi-ruins.jpg',
      summary: 'Sprawling capital of the Vijayanagara Empire along the Tungabhadra river, famed for musical stone pillars and monolithic chariots.',
      highlights: ['Vitthala Stone Chariot', '56 Musical Pillars', 'Royal Enclosures & Stepwells'],
      wireframeAvailable: true,
      audioStoryAvailable: true
    },
    {
      id: 'sanchi-stupa',
      name: 'Great Stupa at Sanchi',
      location: 'Raisen, Madhya Pradesh',
      dynasty: 'Maurya / Shunga Period (3rd BCE)',
      coordinates: [23.4800, 77.7400],
      category: 'Buddhist Architecture',
      rating: 4.8,
      image: '/images/sanchi-stupa.jpg',
      summary: 'One of India\'s oldest stone structures commissioned by Emperor Ashoka, featuring intricately carved gateways (Toranas).',
      highlights: ['Four Ceremonial Torana Gateways', 'Brahmi Inscriptions', 'Asokan Lion Pillar Base'],
      wireframeAvailable: true,
      audioStoryAvailable: true
    },
    {
      id: 'ajanta-caves',
      name: 'Ajanta Rock-Cut Caves',
      location: 'Chhatrapati Sambhajinagar, Maharashtra',
      dynasty: 'Satavahana & Vakataka Empires (2nd BCE - 5th CE)',
      coordinates: [20.5523, 75.7004],
      category: 'Cave Paintings & Sculptures',
      rating: 4.9,
      image: '/images/ajanta-caves.jpg',
      summary: '30 rock-cut Buddhist cave monuments housing masterwork frescoes and colossal rock carvings.',
      highlights: ['Padmapani Mural Fresco', 'Chaitya Prayer Halls', 'Monolithic Sleeping Buddha'],
      wireframeAvailable: true,
      audioStoryAvailable: true
    },
    {
      id: 'konark-sun',
      name: 'Konark Sun Temple',
      location: 'Puri, Odisha',
      dynasty: 'Eastern Ganga Dynasty (1250 CE)',
      coordinates: [19.8876, 85.8896],
      category: 'Solar Chariot Monument',
      rating: 4.8,
      image: '/images/konark-sun.jpg',
      summary: 'Gigantic stone chariot temple dedicated to Surya with 24 carved stone wheels acting as precise sundials.',
      highlights: ['Astronomical Stone Sundials', 'Erotic Kalinga Sculptures', 'Iron Beam Ceiling Engineering'],
      wireframeAvailable: true,
      audioStoryAvailable: true
    },
    {
      id: 'khajuraho',
      name: 'Khajuraho Group of Temples',
      location: 'Chhatarpur, Madhya Pradesh',
      dynasty: 'Chandela Dynasty (950 - 1050 CE)',
      coordinates: [24.8318, 79.9199],
      category: 'Nagara Style Temples',
      rating: 4.8,
      image: '/images/khajuraho.jpg',
      summary: 'Famous for Nagara-style architectural symbolism and expressive sandstone sculptures of life and spirituality.',
      highlights: ['Kandariya Mahadeva Temple', 'Intricate Sandstone Friezes', 'Celestial Apsara Reliefs'],
      wireframeAvailable: true,
      audioStoryAvailable: true
    }
  ],

  // Mobile Tourist Safe Routes, Dedicated Police Stations & Emergency Helplines
  safetyData: {
    destinations: [
      {
        id: 'hampi-ruins',
        name: 'Vitthala Temple Complex (Hampi)',
        state: 'Karnataka',
        startPoint: 'Hampi Bus Station & Tourist Welcome Kiosk',
        destinationPoint: 'Vitthala Temple Eastern Gate',
        safetyScore: 97,
        safetyRating: 'Verified Safe Heritage Corridor',
        distanceKm: '2.4 km',
        estimatedWalkTime: '26 mins walk (or 6 mins Electric Cart)',
        lightingCoverage: '96% Solar Smart LED Illuminated',
        patrolFrequency: 'Every 10 Mins (Karnataka Tourist Police Patrol)',
        crowdDensity: 'Optimal & Family Friendly',
        cctvSurveillance: '48 AI Face & Crowd Monitored Cameras',
        policeStation: {
          name: 'Hampi Heritage Tourist Police Station',
          officerInCharge: 'Inspector R. K. Nayak',
          badgeNumber: 'KA-TP-0482',
          phone: '+91 8394-241222',
          hotline: '112',
          distance: '450m from Monument Entry',
          address: 'Bazaar Road, near Virupaksha Temple Complex, Hampi',
          patrolFleet: '4 Electric Buggies + 2 High-Mobility Patrol Jeeps',
          womenHelpdesk: 'Dedicated 24x7 Women Tourist Support Cell',
          coords: [15.3352, 76.4605]
        },
        safeCheckpoints: [
          { name: 'River Ferry Checkpoint', type: 'Police & Life Guard Post', distance: '0.6 km' },
          { name: 'Bazaar Solar Lighting Hub', type: 'Emergency SOS Alarm Pillar', distance: '1.4 km' },
          { name: 'ASI Heritage Security Post', type: 'First Aid & Authorized Guide Helpdesk', distance: '2.1 km' }
        ],
        navigationSteps: [
          'Depart Tourist Welcome Center via the illuminated Main Carved Street.',
          'Pass 24x7 Police Outpost at 500m (CCTV Surveillance & Help Desk).',
          'Follow the paved river corridor with active solar lighting and SOS poles every 250m.',
          'Arrive at Vitthala Temple secure ticket and check-in pavilion.'
        ],
        googleMapsQuery: 'Hampi Bus Stand to Vitthala Temple Hampi'
      },
      {
        id: 'brihadeeswarar',
        name: 'Brihadeeswarar Great Temple (Thanjavur)',
        state: 'Tamil Nadu',
        startPoint: 'Old Thanjavur Bus Stand / Railway Station',
        destinationPoint: 'Brihadeeswarar Temple Rajagopuram Gate 1',
        safetyScore: 98,
        safetyRating: 'High-Security Cultural Zone',
        distanceKm: '1.4 km',
        estimatedWalkTime: '16 mins walk (or 4 mins Auto-Rickshaw)',
        lightingCoverage: '98% High-Mast Smart LED Lighting',
        patrolFrequency: 'Continuous 24/7 CCTV & 15-min Police Mobile Patrol',
        crowdDensity: 'Active, Safe & Well-Monitored',
        cctvSurveillance: '64 High-Definition 360° Cameras',
        policeStation: {
          name: 'Thanjavur West Tourist Police Station',
          officerInCharge: 'Deputy SP M. Soundararajan',
          badgeNumber: 'TN-TP-1089',
          phone: '+91 4362-230100',
          hotline: '112',
          distance: '300m from Big Temple North Gate',
          address: 'Court Road, Thanjavur Heritage Zone, Tamil Nadu',
          patrolFleet: '3 Highway Patrol Cruisers + 4 Tourist Marshals',
          womenHelpdesk: 'All-Women Police Helpdesk Counter Active',
          coords: [10.7835, 79.1325]
        },
        safeCheckpoints: [
          { name: 'Royal Palace Police Kiosk', type: 'Tourist Police & Lost-Found Post', distance: '0.4 km' },
          { name: 'Sivaganga Park Medical Center', type: 'Emergency First Aid & Paramedic Unit', distance: '0.9 km' },
          { name: 'Heritage Walkway Reception', type: 'Audio Guide & ASI Security Bureau', distance: '1.3 km' }
        ],
        navigationSteps: [
          'Start from Old Bus Stand into the pedestrianized Heritage Heritage Boulevard.',
          'Walk along illuminated royal moat pathway equipped with emergency CCTV call boxes.',
          'Cross the Sivaganga Park Police Post with 24x7 tourist assistance.',
          'Enter through Main Maratha Gateway directly into temple security pavilion.'
        ],
        googleMapsQuery: 'Old Bus Stand Thanjavur to Brihadeeswara Temple'
      },
      {
        id: 'sanchi-stupa',
        name: 'Great Stupa at Sanchi (Raisen)',
        state: 'Madhya Pradesh',
        startPoint: 'Sanchi Railway Station / Gateway Tourist Hotel',
        destinationPoint: 'Stupa 1 Archaeological Enclosure',
        safetyScore: 95,
        safetyRating: 'Protected UNESCO Heritage Zone',
        distanceKm: '1.6 km',
        estimatedWalkTime: '18 mins walk (or 5 mins E-Rickshaw)',
        lightingCoverage: '92% Solar Pathway Illumination',
        patrolFrequency: 'Every 15 Mins (MP Tourist Police & CISF)',
        crowdDensity: 'Serene, Family Friendly & Peaceful',
        cctvSurveillance: '36 High-Zoom Dome Cameras',
        policeStation: {
          name: 'Sanchi Police Post & Tourism Protection Unit',
          officerInCharge: 'Sub-Inspector Anoop Verma',
          badgeNumber: 'MP-TP-0312',
          phone: '+91 7482-266100',
          hotline: '112',
          distance: '500m from Hill Base Ticket Office',
          address: 'Station Road, Sanchi, Raisen District, Madhya Pradesh',
          patrolFleet: '2 Mobile Patrol Vans + ASI Guard Detail',
          womenHelpdesk: 'Tourist Assistance & Mahila Desk',
          coords: [23.4810, 77.7415]
        },
        safeCheckpoints: [
          { name: 'Hill Base Ticket Kiosk', type: 'Police Checkpost & Bag Scanner', distance: '0.5 km' },
          { name: 'Buddhist Monastery Pathway Post', type: 'Solar Lit Emergency Point', distance: '1.1 km' },
          { name: 'Torana Gateway Command Hub', type: 'ASI Central Guard Room', distance: '1.5 km' }
        ],
        navigationSteps: [
          'Exit Sanchi Station on Station Road towards Hill Ascent pathway.',
          'Check in at MP Tourism Information Counter at hill base.',
          'Ascend the paved, well-lit heritage slope protected by guard posts.',
          'Reach the Monument Security Pavilion at South Torana Gateway.'
        ],
        googleMapsQuery: 'Sanchi Railway Station to Great Stupa Sanchi'
      },
      {
        id: 'konark-sun',
        name: 'Konark Sun Temple (Puri)',
        state: 'Odisha',
        startPoint: 'Konark Bus Stand & Yatri Nivas',
        destinationPoint: 'Sun Temple Main Sanctuary Gate',
        safetyScore: 96,
        safetyRating: 'Coastal Cultural Security Zone',
        distanceKm: '1.1 km',
        estimatedWalkTime: '14 mins walk (or 4 mins E-Cart)',
        lightingCoverage: '97% Heritage Streetlighting',
        patrolFrequency: 'Every 12 Mins (Odisha Tourist Police)',
        crowdDensity: 'Active & Welcoming',
        cctvSurveillance: '52 Wide-Angle Panoramic Cameras',
        policeStation: {
          name: 'Konark Marine & Tourist Police Station',
          officerInCharge: 'Inspector P. C. Mohapatra',
          badgeNumber: 'OD-TP-0744',
          phone: '+91 6758-236825',
          hotline: '112',
          distance: '350m from Temple Entrance',
          address: 'Main Ring Road, Konark, Puri District, Odisha',
          patrolFleet: '3 Beach & Monument Cruisers',
          womenHelpdesk: '24x7 Coastal Women & Tourist Aid Desk',
          coords: [19.8885, 85.8905]
        },
        safeCheckpoints: [
          { name: 'Open Air Theatre Security Kiosk', type: 'Tourist Information & Police Desk', distance: '0.3 km' },
          { name: 'Craft Bazaar Surveillance Hub', type: 'First Aid & CCTV Pillar', distance: '0.7 km' },
          { name: 'ASI Sun Temple Entry Complex', type: 'Baggage Scanner & Security Command', distance: '1.0 km' }
        ],
        navigationSteps: [
          'Walk along the paved Konark Grand Plaza towards the craft corridor.',
          'Follow clearly marked tourist signage with LED solar lampposts.',
          'Pass the Marine Police surveillance outpost at plaza crossing.',
          'Arrive at the main ASI security enclosure.'
        ],
        googleMapsQuery: 'Konark Bus Stand to Konark Sun Temple'
      },
      {
        id: 'ajanta-caves',
        name: 'Ajanta Rock-Cut Caves (Chhatrapati Sambhajinagar)',
        state: 'Maharashtra',
        startPoint: 'Ajanta T-Junction Tourist Complex & Amenities Hub',
        destinationPoint: 'Cave 1 Entrance Plaza',
        safetyScore: 95,
        safetyRating: 'Eco-Sensitive High Protection Zone',
        distanceKm: '4.0 km (Eco-Green Bus Route)',
        estimatedWalkTime: '8 mins Eco-Bus ride + 5 mins scenic walkway',
        lightingCoverage: '90% Gorge & Cave Pathway Solar Illumination',
        patrolFrequency: 'Every 15 Mins (Maharashtra Tourist Police & Forest Guards)',
        crowdDensity: 'Controlled Entry (Timed Slots)',
        cctvSurveillance: '40 Low-Light Infrared Cave Cameras',
        policeStation: {
          name: 'Ajanta Caves Special Police Outpost',
          officerInCharge: 'Inspector Sunil Shinde',
          badgeNumber: 'MH-TP-0521',
          phone: '+91 2438-244222',
          hotline: '112',
          distance: '150m from Cave Ticket Plaza',
          address: 'Ajanta Valley Plaza, Chhatrapati Sambhajinagar, Maharashtra',
          patrolFleet: '2 Emergency Valley Evacuation Vehicles + Forest Patrol',
          womenHelpdesk: 'Forest & Tourism Women Guide Desk',
          coords: [20.5530, 75.7012]
        },
        safeCheckpoints: [
          { name: 'T-Junction Amenities Center', type: 'Tourist Police Help Counter & Cloakroom', distance: '0.0 km' },
          { name: 'Waghur River Bridge Outpost', type: 'Safety Checkpoint & Medical Post', distance: '2.5 km' },
          { name: 'Cave Viewpoint Safety Post', type: 'Emergency First Aid & Radiophone Hub', distance: '3.8 km' }
        ],
        navigationSteps: [
          'Board verified Green Eco-Shuttle Bus from T-Junction Amenities Hub.',
          'Ride through protected valley corridor to the Cave Base Station.',
          'Walk up the paved, handrail-equipped cliff staircase with safety netting.',
          'Pass Cave 1 Security Verification checkpoint.'
        ],
        googleMapsQuery: 'Ajanta T Junction to Ajanta Caves Cave 1'
      }
    ],
    emergencyHelplines: [
      { name: 'National Tourist Emergency Helpline', number: '1363 / 1800-11-1363', desc: 'Toll-free 24x7 multi-language guidance & emergency dispatch', category: 'General' },
      { name: 'Police Emergency Response (ERSS)', number: '112', desc: 'Instant police, fire and rescue vehicle dispatch', category: 'Police' },
      { name: 'Central Tourist Police Cell', number: '+91 11-2336-5358', desc: 'Specialized tourist safety, fraud and dispute resolution', category: 'Police' },
      { name: 'National Women Safety Helpline', number: '1091', desc: 'Dedicated 24/7 women emergency responder assistance', category: 'Women Safety' },
      { name: 'Medical Ambulance Support', number: '108', desc: 'Rapid emergency ambulance & first-aid mobile teams', category: 'Medical' },
      { name: 'ASI Monument Security Corps', number: '+91 800-437-4824', desc: 'Archaeological Survey of India monument security & lost property', category: 'Security' }
    ]
  }
};

export default HERITAGE_DATA;
